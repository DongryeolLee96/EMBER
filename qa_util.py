...
def prepare_qa_inputs_rule(data, position, reader):
    ...
    if position=='front':        
        for idx, d in enumerate(data):
            d['answer_{}_str'.format(reader)]=str_exps[idx]+" Answer: "+d['answer_{}'.format(reader)]
            d['answer_{}_weak'.format(reader)]=weak_exps[idx]+" Answer: "+d['answer_{}'.format(reader)]
            d['answer_{}_plain'.format(reader)]=d['answer_{}'.format(reader)]
            
    elif position=='back':            
        for idx, d in enumerate(data):
            d['answer_{}_str'.format(reader)]="Answer: "+d['answer_{}'.format(reader)]+' '+str_exps[idx]
            d['answer_{}_weak'.format(reader)]="Answer: "+d['answer_{}'.format(reader)]+' '+weak_exps[idx]
            d['answer_{}_plain'.format(reader)]=d['answer_{}'.format(reader)]
    else:
        raise TypeError
...
def prepare_qa_inputs_gpt(data, engine, reader):
    ...
    for idx, d in enumerate(data):
        str_inputs1.append(output_revision_prompt.format(em=str_exps[idx] , question=d['question'][0].upper()+d['question'][1:], output=d['answer_{}'.format(reader)] ))
        weak_inputs1.append(output_revision_prompt.format(em=weak_exps[idx] , question=d['question'][0].upper()+d['question'][1:], output=d['answer_{}'.format(reader)] ))
    
    str_outputs1=gpt4_answer(inputs_with_prompts= str_inputs1, engine=engine, max_tokens=300)
    weak_outputs1=gpt4_answer(inputs_with_prompts= weak_inputs1, engine=engine, max_tokens=300)
